...
void LogRegModel::fit(const std::vector<float>& x_values, const std::vector<std::string>& columns, const std::vector<float>& y_values_vec,
    ...
    if (regularization == "L1") {
        throw std::logic_error("L1 regularization for Logistic Regression requires an iterative solver with subgradient methods and is not supported by this basic implementation.");
    }
    if (learning_rate <= 0) {
